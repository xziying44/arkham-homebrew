...
### 修复
- **卡背导出尺寸**：修复出血 DPI 大于 300 时，玩家卡背、遭遇卡背等纯图片卡背不跟随正面尺寸缩放的问题。根因是导出时 DPI 目标尺寸的缩放逻辑位于文字层绘制方法内，而无文字层的卡背会被提前返回从而跳过缩放，停留在 300 DPI 尺寸。

## [3.4.1] - 2026-03-15

### 新增
- **Steam 云上传**：新增与 Cloudinary、ImgBB 同级的 `Steam 云` 上传方式，复用本地 HTTP 服务生成可供 TTS 加载的图片地址，无需额外图床配置。
- **上传页快捷导出**：上传成功后可直接在上传页面点击 `导出TTS物品`，无需返回内容包主页面再次操作。
- **Steam 云教程**：在上传页面内新增 Steam 云上传图文教程，内置操作说明和示意图。

### 修复
- **自动编号**：修复玩家卡自动编号排序规则，支持固定职阶顺序、调查员与签名卡相邻、Bonded 卡跟随主卡、大画卡按基础类型排序，并忽略调查员小卡。
- **大画技能卡**：修复大画技能卡左上角类型文字错误显示为“事件”的问题。
